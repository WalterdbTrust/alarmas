...
sap.ui.define([
	...
	"alarmas/model/formatter",
	"sap/m/MessageBox",
	"sap/m/MessageToast"
], function(BaseController, JSONModel, formatter, MessageBox, MessageToast) {
	"use strict";

	return BaseController.extend("alarmas.controller.Detail", {
		...
		onDelete: function() {
			...
			var oViewModel = this.getModel("detailView"),
				...
				sObjectHeader = this._oODataModel.getProperty(sPath + "/VIN"),
		...
		_onObjectMatched: function(oEvent) {
			...
			this.getModel().metadataLoaded().then(function() {
				var sObjectPath = this.getModel().createKey("P_ALARM_SETTING", oParameter);
		...
		_onBindingChange: function() {
            ...
            this.actulizaDetalle();				
				
		},
		actulizaDetalle: function(){
			var valor = sap.ui.getCore().byId("info_speedActive");
			sap.ui.getCore().byId("info_swSpeedActive").setState(valor.getValue() === "1" ? true : false);

			var valor1 = sap.ui.getCore().byId("info_rpmActive");
			sap.ui.getCore().byId("info_sw_RpmActive").setState(valor1.getValue() === "1" ? true : false);

			var valor2 = sap.ui.getCore().byId("info_tempActive");
			sap.ui.getCore().byId("info_swTempActive").setState(valor2.getValue() === "1" ? true : false);
			
			
			var mySelectMenu = sap.ui.getCore().byId("info_mySelectMenu");
    		...
    		mySelectMenu.bindAggregation("items", { path: '/P_REFERENCE', filters: [new sap.ui.model.Filter('TABLE',sap.ui.model.FilterOperator.EQ,'01')] , template:oItemSelectTemplate}); //			
			
			mySelectMenu.setSelectedKey(("00" + sap.ui.getCore().byId("info_rpmValue").getValue()).slice(-2));
		},
