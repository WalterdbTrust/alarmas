sap.ui.define([
	...
	return BaseController.extend("alarmas.controller.Master", {
		...
		onInit: function() {
			...
			oList.attachEventOnce("updateFinished", function() {
				...
				oModel.attachRequestCompleted(function onCompleted(oEvent) {
					...
					that._oList.getBinding("items").filter(filtro1);
					that._oList.getBinding("items").attachDataReceived(function() {
						var marrayOri = that._oList.getBinding("items").getContexts();
						if (marrayOri.length === 0) {
							sap.m.MessageBox.information("Lo siento, no se encontro vehiculo para la cuenta " + userEmail);
							return;
						}
						var sObjectId = marrayOri[0].getObject().VIN;
						
						var mesCelular = $(window).width() < 800 ? true : false;
						if (marrayOri.length === 1 || !mesCelular) {
    						that.getRouter().navTo("object", {
    							VIN: sObjectId
    						}, true);
						}
						
						that._oList.setProperty("visible", true);
					});
				});
...
// 			this._findItem();
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("VIN", FilterOperator.Contains, sQuery)];
			} else {
		...
		_onMasterMatched: function() {
			this._oListSelector.oWhenListLoadingIsDone.then(
				...
				// 			VIN: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("VIN"))
				// 		}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				VIN: encodeURIComponent(oItem.getBindingContext().getProperty("VIN"))
			}, bReplace);
		},
